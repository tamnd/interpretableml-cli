...
describes what users get. Pick a format with `-o`, or let iml choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
iml <command> -o table   # aligned columns for reading
iml <command> -o jsonl   # one JSON object per line, for piping
iml <command> -o json    # a single JSON array
iml <command> -o csv     # spreadsheet friendly
iml <command> -o tsv     # tab-separated
iml <command> -o url     # just the URL column
iml <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
iml <command> --fields id,title,url
```
...
```bash
iml <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
iml <command>            # a table, because this is a terminal
iml <command> | wc -l    # JSONL, because this is a pipe
```
